...
public final class Base {
    ...
    private static void setBrowser(){
        ...
        switch (binary){
            case "webkit":
                browser.set( getPlaywright().webkit().launch(new BrowserType.LaunchOptions().setChannel(channel).setHeadless(Boolean.parseBoolean(headless))));
                break;
            case "firefox":
                browser.set( getPlaywright().firefox().launch(new BrowserType.LaunchOptions().setChannel(channel).setHeadless(Boolean.parseBoolean(headless))));
                break;
            default:
                browser.set( getPlaywright().chromium().launch(new BrowserType.LaunchOptions().setChannel(channel).setHeadless(Boolean.parseBoolean(headless))));
                break;
        }
    }
